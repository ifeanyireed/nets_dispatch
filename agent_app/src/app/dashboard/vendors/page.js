...
export default function Vendors() {
  const vendors = [
    { name: "Chicken Republic", category: "Restaurant", location: "Lekki Phase 1", activeOrders: "24", status: "Active", statusColor: "text-live bg-live/10 border border-live/20" },
    { name: "ShopRite", category: "Groceries", location: "Victoria Island", activeOrders: "12", status: "Active", statusColor: "text-live bg-live/10 border border-live/20" },
    { name: "MedPlus Pharmacy", category: "Health", location: "Ikeja", activeOrders: "8", status: "Busy", statusColor: "text-hazard bg-hazard/10 border border-hazard/20" },
    { name: "Fola Foods", category: "Restaurant", location: "Yaba", activeOrders: "0", status: "Closed", statusColor: "text-text-1 bg-text-2/10 border border-hairline-2" },
  ];
    ...
    <div className="flex-1 flex flex-col min-w-0">
      <div className="p-6 md:p-8 flex-1 flex flex-col">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className="font-mono text-xs tracking-[0.15em] text-hazard uppercase">Vendor Management</span>
            </div>
            <h1 className="font-display font-extrabold uppercase text-4xl leading-none tracking-wide text-text-0">Vendors Network</h1>
          </div>
        ...
        <div className="flex items-center gap-3 bg-panel border border-hairline rounded-full px-4 py-3 mb-6 focus-within:border-hazard transition-colors">
          ...
          <input 
            type="text" 
            placeholder="Search by Vendor name, branch, or category..." 
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {["All Vendors", "Restaurants", "Groceries", "Health", "Active", "Closed"].map((chip, i) => (
        ...
        <div className="bg-panel border border-hairline rounded-2xl overflow-hidden flex-1">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-panel-2 border-b border-hairline">
                  ...
                  <th className="px-6 py-4 font-mono text-[10px] uppercase tracking-widest text-text-2 font-normal">Category</th>
                  <th className="px-6 py-4 font-mono text-[10px] uppercase tracking-widest text-text-2 font-normal">Location</th>
                  <th className="px-6 py-4 font-mono text-[10px] uppercase tracking-widest text-text-2 font-normal">Status</th>
                  <th className="px-6 py-4 font-mono text-[10px] uppercase tracking-widest text-text-2 font-normal text-right">Active Orders</th>
                </tr>
              </thead>
                ...
                {vendors.map((vendor, idx) => (
                  <tr key={idx} className="hover:bg-panel-2/50 transition-colors cursor-pointer group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-md bg-panel-2 border border-hairline flex items-center justify-center font-bold text-text-0 font-sans text-xs">
                          {vendor.name.charAt(0)}
                        </div>
                        <span className="font-semibold text-text-0 group-hover:text-hazard transition-colors">{vendor.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-text-1">{vendor.category}</td>
                    <td className="px-6 py-4 text-text-1">{vendor.location}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${vendor.statusColor}`}>
                        {vendor.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right font-mono font-semibold text-text-0">{vendor.activeOrders}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="p-4 border-t border-hairline flex items-center justify-between text-xs font-mono text-text-2">
            <span>Showing 4 of 48 vendors</span>
